...
export function runRelated(ctx) {
    ...
    const symbol = db
        .prepare(`SELECT s.id, s.name, f.path FROM symbols s JOIN files f ON f.id = s.file_id
    ...
    const outgoing = db
        .prepare(`SELECT relationship_type, description, status, to_type AS other_type, to_id AS other_id
       FROM relationships WHERE from_type = 'symbol' AND from_id = ?`)
        .all(symbol.id);
    const incoming = db
        .prepare(`SELECT relationship_type, description, status, from_type AS other_type, from_id AS other_id
       FROM relationships WHERE to_type = 'symbol' AND to_id = ?`)
        .all(symbol.id);
    const resolveName = (type, id) => {
        if (type === "symbol") {
            const row = db.prepare(`SELECT name FROM symbols WHERE id = ?`).get(id);
            return row?.name ?? `#${id}`;
        }
        const table = `${type}s`;
        try {
    ...
    const capabilities = db
        .prepare(`SELECT c.name FROM capability_symbols cs JOIN capabilities c ON c.id = cs.capability_id WHERE cs.symbol_id = ?`)
        .all(symbol.id);
    const flowSteps = db
        .prepare(`SELECT DISTINCT fl.name FROM flow_steps fst JOIN flows fl ON fl.id = fst.flow_id WHERE fst.symbol_id = ?`)
        .all(symbol.id);
    ...
    if (outgoingNamed.length > 0) {
        ...
        for (const r of outgoingNamed)
            console.log(`- --[${r.relationship_type}]--> ${r.other_type}:${r.other_name}${r.description ? `  (${r.description})` : ""}`);
    }
