...
function getCapability(db, name) {
    return db.prepare("SELECT * FROM capabilities WHERE name = ?").get(name);
}
function renderCapability(cap) {
    if (!cap)
        return ["_Not cataloged._", ""];
    const lines = [
        "**Status:** " +
            cap.status +
            "  **Category:** " +
    ...
    if (cap.implementation_summary)
        lines.push("> " + cap.implementation_summary, "");
    return lines;
}
function renderFlow(db, name) {
    const rows = db
...
function renderModuleTable(db, names) {
    const lines = [
...
function findingsBlock(db, category) {
    const rows = (category
...
function openQuestionsBlock(db, category) {
    const rows = (category
        ? db.prepare("SELECT * FROM open_questions WHERE category = ? ORDER BY priority").all(category)
        : db.prepare("SELECT * FROM open_questions ORDER BY priority").all());
    if (rows.length === 0)
        return ["_None recorded._", ""];
    const lines = [];
    for (const r of rows) {
        lines.push("- [" + (r.priority ?? "?") + "] " + r.question);
        if (r.likely_interpretation)
            lines.push("  Likely: " + r.likely_interpretation);
    }
    lines.push("");
    return lines;
}
...
function writeTopLevelDoc(name, lines) {
    writeFileSync(path.join(CATALOG_ROOT, name), lines.join("\n") + "\n", "utf8");
    console.log("Wrote " + name);
}
function generateOpenQuestionsDoc(db) {
    const rows = db
    ...
    for (const row of rows) {
        ...
        if (row.evidence_inspected)
            lines.push("- **Evidence inspected:** " + row.evidence_inspected);
        if (row.reason_unresolved)
            lines.push("- **Why unresolved:** " + row.reason_unresolved);
        if (row.likely_interpretation)
            lines.push("- **Likely interpretation:** " + row.likely_interpretation);
        if (row.verification_method)
            lines.push("- **How to verify:** " + row.verification_method);
        lines.push("");
    }
...
function main() {
    ...
    try {
        ...
        const memorySystemRows = db.prepare("SELECT * FROM memory_map").all();
        ...
        const skillRows = db.prepare("SELECT * FROM skill_map").all();
        ...
        const extractionRows = db
            .prepare("SELECT name, kind, file_path, importance, application_coupling FROM extraction_candidates")
            .all();
        const extractionLines = extractionRows.length > 0
