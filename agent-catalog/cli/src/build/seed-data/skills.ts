import type { SeedDataType, SeedSkill, SeedEvidence, SeedOpenQuestion } from "../seed-types.js";
...
export const skillOpenQuestions: SeedOpenQuestion[] = [
  {
    ...
    evidenceInspected:
      "src/skills/types.ts's SkillUsagePath.readPath field ('Path visible to the tool runtime when it reads SKILL.md') suggests lazy, tool-mediated reads rather than eager full-content injection, but this is inferred from a field comment, not confirmed by reading the loader/runtime code.",
    reasonUnresolved:
      "src/skills/loading and src/skills/runtime subdirectories were confirmed to exist but not opened this pass.",
    likelyInterpretation:
      "Likely a hybrid: a short skill index/description is always in context (for model-selection), with the full SKILL.md body fetched on demand via a tool when the skill is actually invoked -- consistent with the 'Skill' tool pattern referenced in root AGENTS.md ('Skills own workflows').",
    verificationMethod:
      "Read src/skills/loading/*.ts and src/skills/runtime/*.ts in full, and find the model-facing tool (if any) that reads a SKILL.md body on demand.",
    priority: "high",
    status: "open",
  },
  {
    ...
    likelyInterpretation:
      "Likely description-based: a compact per-skill description is included in the system prompt or a dedicated Skill tool's own description, and the model chooses by issuing a tool call naming the skill.",
    verificationMethod:
      "Read src/skills/discovery/*.ts and search for where skill descriptions are assembled into a prompt or tool schema.",
    priority: "medium",
    status: "open",
  },
];
