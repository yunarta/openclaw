...
export function runSymbol(ctx: CommandContext): void {
  ...
  const rows = db.prepare(sql).all(...(params as [])) as unknown as SymbolRow[];
  ...
  const capStmt = db.prepare(
    `SELECT c.name AS capability_name, cs.role FROM capability_symbols cs
